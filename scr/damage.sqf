...
//_EH = (finddisplay 46) displayaddeventhandler ["KeyDown","if ((_this select 1) == (ActionKeys ""user19"")) then {_as = [] execVM ""\IL76TD\scr\Rampdown.sqs""}"];
...
/*moduleName_keyDown =
{
	...
	{
		nul = [_vehicle] execVM "\IL76TD\scr\Rampdown.sqs";
	};
};
...
while {damage _vehicle < 1} do
{
	...
	/*(findDisplay 46) displayAddEventHandler ["KeyDown", "call" + str {
		...
		{
			[_vehicle] execVM "\IL76TD\scr\Rampdown.sqs";
		};
	}];*/
	/*_vehicle vehiclechat format ["gear_r1 %1",_vehicle animationPhase "gear_r1"];
	_vehicle vehiclechat format ["anim %1",_anim];*/
	if (((_vehicle animationPhase "gear_r1") < 0.5) and (_anim == 1) and (_flag == 0)) then
	{
		//_vehicle vehiclechat format ["animuuu %1",_anim];
		//_vehicle vehiclechat (localize "STR_IL76TD_SV");
		_vehicle vehicleRadio "IL76tdGearDown";
		_flag = 1;
	};
	if (((_vehicle animationPhase "gear_r1") > 0.9) and (_anim == 0) and (_flag == 0)) then
	{
		//_vehicle vehiclechat format ["animuuu %1",_anim];
		//_vehicle vehiclechat (localize "STR_IL76TD_SU");
		_vehicle vehicleRadio "IL76tdGearUp";
		_flag = 1;
	};
	//sleep 0.01;
	...
	{
		_gen = 1;
		_vehicle vehicleRadio "IL76tdgenerator";
	};

	if ((damage _vehicle > 0.2) and (_hyd == 0)) then
	{
		_hyd = 1;
		_vehicle vehicleRadio "IL76tdhydraulics";
	};

	if ((damage _vehicle > 0.3) and (_nsw == 0)) then
	{
		_nsw = 1;
		_vehicle vehicleRadio "IL76tdno_swap";
	};

	if ((damage _vehicle > 0.4) and (_vib == 0)) then
	{
		_vib = 1;
		_vehicle vehicleRadio "IL76tdvibration";
	};

	if ((damage _vehicle > 0.5) and (_ads == 0)) then
	{
		_ads = 1;
		_vehicle vehicleRadio "IL76tdair_dt_sys";
	};

	if ((damage _vehicle > 0.6) and (buster == 0)) then
	{
		buster = 1;
		_vehicle vehicleRadio "IL76tdbstr_malf";
	};

	if ((damage _vehicle > 0.7) and (_all == 0)) then
	{
		_all = 1;
		_vehicle vehicleRadio "IL76tdfire_all";
	};
	...
	while {_countd < _countmbd} do
	{
		if (_sayed < 1) then
		{
			_vehicle vehicleRadio "IL76tdbeep";
			_sayed = 1;
		};
		...
		if (_countnd > 0 ) then
		{
			...
			sleep 0.01;
			//_vehicle vehiclechat format [((localize "STR_IL76TD_FIR") + "%1" + (localize "STR_IL76TD_ENG")),_element];
			switch (_element) do
			{
				case (1): {_vehicle vehicleRadio "IL76tdfire_1";};
				case (2): {_vehicle vehicleRadio "IL76tdfire_2";};
				case (3): {_vehicle vehicleRadio "IL76tdfire_3";};
				case (4): {_vehicle vehicleRadio "IL76tdfire_4";};
			};
	...
	sleep (0.03 + _zloop);
	[_vehicle,_damaged] execVM "\IL76TD\scr\enginesmokes.sqf";
